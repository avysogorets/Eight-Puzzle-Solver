...
import structures.ManhattanComparator;
import structures.YAstate;

public class ManhattanSearch 
{
	protected EightPuzzle puzzle;
	protected ManhattanComparator comparator;
	...
	public ManhattanSearch (EightPuzzle puzzle)
	{
		this.puzzle = puzzle;
	...
	public List<List<Integer>> findSolution()
	{
		...
		solutionStatsAsList.add(puzzle.getManhattan(heap.peek().getState()));
        ...
        if(!heap.isEmpty())
        {
        	...
        	while(!state.equals(puzzle.getInitialState()))
        	{
        		...
        		solutionStatsAsList.add(puzzle.getManhattan(predecessor));
        		state = predecessor;
        	}
